...
for mode in ["color", "bw"]:
    ...
    axes[1].set_ylim(1e-6, 1e20)
    axes[1].set_yscale("log")
    axes[1].grid(True)

    handles, labels = axes[0].get_legend_handles_labels()
    # 凡例はまとめて一番下に
    fig.legend(handles, labels,loc="lower center", bbox_to_anchor=(0.5, 0.02), ncol=3, handletextpad=0.5, columnspacing=0.5)
    plt.tight_layout(rect=[0, 0.05, 1, 1])  # 凡例のスペース確保
    plt.savefig(f"3comparison_plots{mode_suffix}.pdf")
    plt.close()
