...
def clip_invalid_values(y, min_val=1e-10, max_val=1e20):
    y = np.array(y)
...
for mode in ["color", "bw"]:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"step_size_plot{mode_suffix}.pdf")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"function_value_plot{mode_suffix}.pdf")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"gradient_norm_plot{mode_suffix}.pdf")
